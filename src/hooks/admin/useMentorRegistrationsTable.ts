...
import type { MenteeMentorRegistration } from '@/types/registration';
import type { GridSortModel, GridPaginationModel } from '@mui/x-data-grid';
import { useToast } from '@/providers/hooks/useToast';

export interface MentorRegistrationsTableState {
  registrations: MenteeMentorRegistration[];
  loading: boolean;
  ...
  viewRegistration: MenteeMentorRegistration | null;
  viewLoading: boolean;
}

export const useMentorRegistrationsTable = () => {
  const [state, setState] = useState<MentorRegistrationsTableState>({
    registrations: [],
    loading: false,
  ...
  const fetchRegistrations = useCallback(() => {
    ...
    try {
      ...
      const response = registrationApi.getMockMentorRegistrations();
      
      /* Uncomment when real API is available
      const response = await registrationApi.listMentorRegistrations({
        page: state.page + 1, // API pagination is 1-based, DataGrid is 0-based
        pageSize: state.pageSize,
        search: state.searchQuery,
        sort: state.sortModel[0]?.sort === 'desc' ? 'desc' : 'asc',
        name: state.sortModel[0]?.field as keyof MenteeMentorRegistration,
      });
      */
  ...
  const handleViewClick = async (registrationId: number) => {
    ...
    try {
      ...
      if (registration) {
        ...
        /* Uncomment when real API is available
        const response = await registrationApi.getMentorRegistration(registrationId);
